...
def test_gate_unitary_correct(op, op_name):
    ...
    assert np.allclose(unitary, unitary_expected, atol=1e-03)
...
def test_arbitrary_unitary_correct():
    """Test if lightning.gpu correctly applies an arbitrary unitary by reconstructing its
    matrix"""
    wires = 2
    dev = qml.device("lightning.gpu", wires=wires)
    ...
    assert np.allclose(unitary, random_unitary, atol=1e-06)
...
def test_arbitrary_inv_unitary_correct():
    ...
    def output(input):
        ...
        qml.adjoint(qml.QubitUnitary(random_unitary, wires=range(2)).inv())
        return qml.state()
    ...
    assert np.allclose(unitary, random_unitary_inv, atol=1e-06)
