 ...
 */
template <class T = cuFloatComplex, class DevTypeID = int>
inline auto scaleAndAddC_CUDA(const T a, const T *v1, const T *v2, const int data_size,
    ...
    if constexpr (std::is_same_v<T, cuComplex>) {
        cublasCaxpy(handle, data_size, a, v1, 1, v2, 1);
    } else if constexpr (std::is_same_v<T, cuDoubleComplex>) {
        cublasZaxpy(handle, data_size, a, v1, 1, v2, 1);
    }
    cublasDestroy(handle);
}
