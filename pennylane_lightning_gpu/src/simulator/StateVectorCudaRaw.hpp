...
class StateVectorCudaRaw
  ...
  public:
    ...
    StateVectorCudaRaw(
        CFP_t *gpu_data, size_t num_qubits, const DevTag<int> &dev_tag,
        bool alloc = true,
    ...
    StateVectorCudaRaw(const CFP_t *gpu_data, size_t length)
        : StateVectorCudaRaw(Util::log2(length)) {
    ...
    StateVectorCudaRaw(
        ...
        SharedCublasCaller cublascaller_in = make_shared_cublas_caller(),
        SharedCusparseHandle cusparsehandle_in = make_shared_cusparse_handle())
        : StateVectorCudaRaw(Util::log2(length), dev_tag, true,
                             std::move(handle_in), std::move(cublascaller_in),
                             std::move(cusparsehandle_in)) {
        BaseType::CopyGpuDataToGpuIn(gpu_data, length, false);
    }

    StateVectorCudaRaw(const std::complex<Precision> *host_data, size_t length)
        : StateVectorCudaRaw(Util::log2(length)) {
        BaseType::CopyHostDataToGpu(host_data, length, false);
    }

    StateVectorCudaRaw(const StateVectorCudaManaged &other)
