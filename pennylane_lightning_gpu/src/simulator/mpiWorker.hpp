...
// Get MPI communicator type
inline auto getCommunicatorType() -> std::string {
    char lib_name[MPI_MAX_LIBRARY_VERSION_STRING];
    int lib_name_len;
    MPI_Get_library_version(lib_name, &lib_name_len);
    std::string lib_name_str(lib_name);
    // Remove white space
    lib_name_str.erase(
        std::remove_if(lib_name_str.begin(), lib_name_str.end(),
                       [](unsigned char ch) { return std::isspace(ch); }),
        lib_name_str.end());
    // Convert all letter to lowercase
    std::transform(lib_name_str.begin(), lib_name_str.end(),
                   lib_name_str.begin(),
                   [](unsigned char ch) { return std::tolower(ch); });

    std::vector<std::string> toFind = {"mpich", "openmpi"};
    std::string commType;
    for (size_t i = 0; i < toFind.size(); i++) {
        if (lib_name_str.find(toFind[i]) != std::string::npos) {
            commType = toFind[i];
        }
    }
    return commType;
...
/*
// Create bit pairs for bit index swap operation
inline auto bitPairsToSwap(int numLocalQubits, std::vector<int> &Wires)
    -> std::vector<int2> {
    std::vector<int> wires(Wires);
    std::sort(wires.begin(), wires.end());
    std::size_t numLocalWires = std::count_if(
        wires.begin(), wires.end(), [&](int n) { return n < numLocalQubits; });

    std::vector<std::size_t> localavailwires(numLocalQubits);
    std::iota(localavailwires.begin(), localavailwires.end(), 0);

    for (size_t i = 0; i < numLocalWires; i++) {
        localavailwires.erase(localavailwires.begin() + wires[i]);
    }

    std::vector<int2> wirepairs;
    // Make bit pairs to swap. Need to find a best way for performance
    // purpose.
    for (std::size_t i = numLocalWires; i < wires.size(); i++) {
        std::size_t j = i - numLocalWires;
        int2 wirepair = make_int2(
            static_cast<int>(wires[i]),static_cast<int>(localavailwires[j])
            //static_cast<int>(localavailwires[localavailwires.size() - 1 - j])
            );
...
// Create local target and control wires
inline void createLocalOperationWires(std::vector<int> &wires,
                                      std::vector<int2> &wirePairs) {
    for (size_t i = 0; i < wires.size(); i++) {
        for (size_t j = 0; j < wirePairs.size(); j++) {
            if (wires[i] == wirePairs[j].x) {
                wires[i] = wirePairs[j].y;
            }
        }
    }
}

// Get wire pairs for MPI swap operation and corresponding local target and
// control wires
inline std::tuple<bool, std::vector<int2>, std::vector<int>, std::vector<int>>
createOperationWires(int numLocalQubits, const std::vector<int> &ctrlsInt,
                     const std::vector<int> &tgtsInt) {
    bool BSwapReq = true;
    std::vector<int> Wires(tgtsInt);
    Wires.insert(Wires.end(), ctrlsInt.begin(), ctrlsInt.end());
    auto wirePairs = bitPairsToSwap(numLocalQubits, Wires);
    ...
    if (wirePairs.size() == 0) {
        BSwapReq = false;
        return {BSwapReq, wirePairs, localCtrls, localTgts};
    } else {
        createLocalOperationWires(localCtrls, wirePairs);
        createLocalOperationWires(localTgts, wirePairs);
        return {BSwapReq, wirePairs, localCtrls, localTgts};
    }
}
...
// Get wire pairs for MPI swap operation and corresponding local target and
// control wires
inline std::tuple<bool, std::vector<int2>, std::vector<int>>
createOperationWires(int numLocalQubits, const std::vector<int> &tgtsInt) {
    bool BSwapReq = true;
    std::vector<int> Wires(tgtsInt);
    auto wirePairs = bitPairsToSwap(numLocalQubits, Wires);
    ...
    if (wirePairs.size() == 0) {
        BSwapReq = false;
        return {BSwapReq, wirePairs, localTgts};
    } else {
        createLocalOperationWires(localTgts, wirePairs);
        return {BSwapReq, wirePairs, localTgts};
    }
...
inline bool IsIndexSwapRequired(int numLocalQubits, int numTotalQubits,
    ...
    for (size_t i = 0; i < ctrlsInt.size(); i++) {
        totalWires[ctrlsInt[i]] = Target;
    }

    for (size_t i = 0; i < tgtsInt.size(); i++) {
        totalWires[tgtsInt[i]] = Control;
    }
...
inline SharedMPIWorker make_shared_mpi_worker(custatevecHandle_t handle,
    ...
    int rank, numProcs;
    PL_MPI_IS_SUCCESS(MPI_Comm_size(mpiCommunicator, &numProcs));
    PL_MPI_IS_SUCCESS(MPI_Comm_rank(mpiCommunicator, &rank));
    ...
    std::string mpi_lib_type = getCommunicatorType();

    if (mpi_lib_type == "mpich") {
        communicatorType = CUSTATEVEC_COMMUNICATOR_TYPE_MPICH;
    }

    if (mpi_lib_type == "openmpi") {
    ...
    if (nP2PDeviceBits != 0) {
        ...
        std::vector<cudaIpcMemHandle_t> ipcMemHandles(numProcs);
        ...
        std::vector<cudaIpcEventHandle_t> ipcEventHandles(numProcs);
        ...
        int p2pSubSVIndexBegin = (rank / nSubSVsP2P) * nSubSVsP2P;
