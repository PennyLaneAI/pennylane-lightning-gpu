...
TEST_CASE("AdjointJacobianGPU::AdjointJacobianGPU Op=PauliRot-RX, Obs=Z",
          "[AdjointJacobianGPU]") {
    AdjointJacobianGPU<double> adj;
    std::vector<double> param{-M_PI / 7, M_PI / 5, 2 * M_PI / 3};

    {
        ...
        for (const auto &p : param) {
            auto ops = adj.createOpsData({"PauliRot"}, {{p}}, {{"RX"}}, {{0}},
                                         {false});
...
TEST_CASE("AdjointJacobianGPU::adjointJacobian Op=PauliRot-RY, Obs=X",
          "[AdjointJacobianGPU]") {
    AdjointJacobianGPU<double> adj;
    std::vector<double> param{-M_PI / 7, M_PI / 5, 2 * M_PI / 3};
    {
        ...
        for (const auto &p : param) {
            auto ops = adj.createOpsData({"PauliRot"}, {{p}}, {{"RY"}}, {{0}},
                                         {false});
...
TEST_CASE("AdjointJacobianGPU::adjointJacobian Op=PauliRot-RX, Obs=[Z,Z]",
          "[AdjointJacobianGPU]") {
    AdjointJacobianGPU<double> adj;
    std::vector<double> param{-M_PI / 7, M_PI / 5, 2 * M_PI / 3};
    {
        ...
        auto ops = adj.createOpsData({"PauliRot"}, {{param[0]}}, {{"RX"}},
                                     {{0}}, {false});
...
TEST_CASE(
    "AdjointJacobianGPU::adjointJacobian Op=PauliRot-[RX,RX,RX], Obs=[Z,Z,Z]",
    "[AdjointJacobianGPU]") {
    AdjointJacobianGPU<double> adj;
    std::vector<double> param{-M_PI / 7};
    {
        const size_t num_qubits = 3;
        const size_t num_params = 3;
        const size_t num_obs = 3;
        ...
        auto ops =
            adj.createOpsData({"PauliRot"}, {{param[0]}}, {{"RX", "RX", "RX"}},
                              {{0, 1, 2}}, {false});

        adj.adjointJacobian(psi.cuda_sv.getData(), psi.cuda_sv.getLength(),
                            jacobian, {obs1, obs2, obs3}, ops, {0}, true);

        CAPTURE(jacobian);
        CHECK(-sin(param[0]) == Approx(jacobian[0][0]).margin(1e-7));
    }
}
TEST_CASE("AdjointJacobianGPU::adjointJacobian Op=PauliRot-Mixed, Obs=[XXX]",
          "[AdjointJacobianGPU]") {
    AdjointJacobianGPU<double> adj;
    std::vector<double> param{-M_PI / 7, M_PI / 5, 2 * M_PI / 3};
    {
        ...
        adj.adjointJacobian(psi.cuda_sv.getData(), psi.cuda_sv.getLength(),
                            jacobian, {obs}, ops, {0, 1, 2, 3}, true);
        CAPTURE(jacobian);

        // Computed with PennyLane using default.qubit.adjoint_jacobian
        CHECK(0.0 == Approx(jacobian[0][0]).margin(1e-7));
        CHECK(-0.3644495628 == Approx(jacobian[0][1]).margin(1e-7));
        CHECK(-0.458626454 == Approx(jacobian[0][2]).margin(1e-7));
        CHECK(-0.1275152315 == Approx(jacobian[0][3]).margin(1e-7));
        CHECK(0.0 == Approx(jacobian[0][4]).margin(1e-7));
        CHECK(0.0 == Approx(jacobian[0][5]).margin(1e-7));
    }
}
