...
class SparseHamiltonianGPU final : public ObservableGPU<T> {
  ...
  public:
    ...
    void applyInPlace(SVType<T> &sv) const override {
        ...
        // clang-format off
        DataBuffer<IdxT> H_csrOffsets  { offsets_.size(), device_id, stream_id,
true}; DataBuffer<IdxT> H_columns     { indices_.size(), device_id, stream_id,
true}; DataBuffer<CFP_t> H_values     { data_.size(),    device_id, stream_id,
true};

        // Transfer ownership after state update to StateVectorCudaManaged
        std::unique_ptr<DataBuffer<CFP_t>> d_sv_prime =
std::make_unique<DataBuffer<CFP_t>>(length, device_id, stream_id, true);

        H_csrOffsets.CopyHostDataToGpu( offsets_.data(),
H_csrOffsets.getLength(), false); H_columns.CopyHostDataToGpu( indices_.data(),
H_columns.getLength(),    false); H_values.CopyHostDataToGpu(     data_.data(),
H_values.getLength(),     false);
        // clang-format on
